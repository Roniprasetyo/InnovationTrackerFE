...
export default function MasterTypeEdit({ onChangePage, withID }) {
  ...
  const userSchema = object({
    typId: number(),
    typName: string().max(50, "50 characters max").required("required"),
  });
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const data = await UseFetch(API_LINK + "MasterType/GetDataTypeById", {
          id: withID,
        });
    ...
    if (Object.values(validationErrors).every((error) => !error)) {
      ...
      try {
        const data = await UseFetch(
          API_LINK + "MasterType/UpdateType",
          formDataRef.current
        );
        ...
        <form onSubmit={handleAdd}>
          <div className="card mb-5">
            <div className="card-header p-2">
              <h2 className="fw-bold text-center">Type Form</h2>
            </div>
                ...
                <div className="row mt-4">
                  <div className="col-lg-6">
                    <Input
                      ...
                      label="Name"
                      isRequired
                      value={formDataRef.current.typName}
                      onChange={handleInputChange}
                      errorMessage={errors.typName}
                    />
                  </div>
                </div>
              )}
